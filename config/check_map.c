...
int check_map_chars(t_map *map)
{
    ...
    while (map->map[i]) {
        ...
        while(map->map[i][j] == ' ' || map->map[i][j] == '\t')
                j++;
        index = j;
        if(map->map[i][index] != '1')
            ft_exit("Error: first character in the map\n", map);
        while (map->map[i][j]) {
            if (map->map[i][j] != '1' && map->map[i][j] != '0' && map->map[i][j] != ' ' && map->map[i][j] != 'N' && map->map[i][j] != 'S' && map->map[i][j] != 'W' && map->map[i][j] != 'E')
                ft_exit("Error: Unexpected character in the map!\n", map);
            if(map->map[i][j] == 'S' ||  map->map[i][j] == 'N' || map->map[i][j] == 'E' || map->map[i][j] == 'W')
                player_count++;
            j++;
        }
        j = 0;
    ...
    if(player_count != 1)
        ft_exit("Error: The map need one player\n", map);
